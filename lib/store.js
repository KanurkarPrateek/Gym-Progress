...
function userFor(req) {
  const list = accounts();
  if (list.length === 0) {
    // Configured but unparseable (a typo in USERS) must lock the site, not open it.
    if ((process.env.USERS || '').trim()) return null;
    return { name: 'local', accent: FALLBACK_ACCENTS[0], open: true };
  }
